...
[assets]     Hash of JavaScript and CSS assets to load <code>{ :javascripts => [], :stylesheets => [] }</code>
...
To manually mark a link as traditional, pass <code>:traditional => true</code> or <code>:data-deep-link => nil</code>.
...
Use the <tt>ajax_header</tt> helper in your controllers or views to add data to the <tt>Ajax-Info</tt> header.  Values are converted to JSON before being sent over the wire.  Internally this function uses <code>Ajax.set_header</code>.

You can use <code>Ajax.get_header</code> to inspect <tt>Ajax-Info</tt> header values.  See the <b>In Views</b> example.
